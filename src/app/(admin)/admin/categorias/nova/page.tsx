...
import { createCategoryAction } from "@/actions/admin/catalog";
...
export default async function NovaCategoriaPage() {
  ...
  return (
    <div>
      <AdminHeader title="Nova Categoria" />
      <div className="p-6 max-w-2xl">
        ...
        <form action={createCategoryAction} className="space-y-6">
          <section className="rounded-xl border border-border bg-card p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div className="sm:col-span-2">
                ...
                <input name="name" type="text" required placeholder="ex: Eletrônicos" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5">Ícone (emoji)</label>
                <input name="icon" type="text" placeholder="📱" maxLength={4} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>

              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-foreground mb-1.5">Categoria Pai</label>
                <select name="parent_id" className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
              ...
              <div className="sm:col-span-2">
                ...
                <textarea name="description" rows={3} placeholder="Descrição da categoria…" className="w-full resize-none rounded-lg border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
          ...
          <div className="flex justify-end gap-3">
            <Link href="/admin/categorias" className="rounded-lg border border-border px-5 py-2.5 text-sm font-medium text-foreground hover:bg-muted transition-colors">
              Cancelar
            </Link>
